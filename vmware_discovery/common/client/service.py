...
class AbstractService(object):
    ...
    def _patch(self, client):
        try:
            ...
            module = (importutils.
                      import_module("vmware.common.client.patch.%s" %
                                    (self.base_name)))
...
class KeystoneService(AbstractService):
    ...
    def new_client(self):
        return self.clazz(**self.base_args)
...
class GlanceService(AbstractService):
    ...
    def new_client(self):
        ...
        if not url.endswith('/'):
            url += '/'
        return self._patch(self.clazz(url, token=self.keystone.auth_token,
...
class ClientServiceCatalog(object):
    ...
    def new_client(self, svc_type):
        ...
        if service_versions:
            return service_versions[0].new_client()
        return None
    ...
    def get_client(self, svc_type, version_filter=None):
        ...
        if response_json is not None:
            if 'version' in response_json:
                ...
                for version_meta in versions:
                    if 'status' in version_meta and version_meta['status'] == 'CURRENT':
                        ver = version_meta['id']
    ...
    def _discover_services(self):
        ...
        for svc_type in public_eps.keys():
            ...
            for entry in public_eps[svc_type]:
                # If a target region is configured, skip any endpoints that don't belong to it
                if target_region and entry['region'] != target_region:
                    continue
